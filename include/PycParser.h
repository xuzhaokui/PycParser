...
public:
	PyObject* Parse(const char* fileName)
	{
		...
		//open xml file writer, xxx.pyc->xxx.xml
...
private:
	int ReadMagicNumber()
	{
		int magic = m_Reader.ReadInt(m_File);
		printf("[Info] : MAGIC NUMBER 0x%x\n",magic);
		return magic;
	}
		...
		case TYPE_STRING:
			{
				...
				if(type == TYPE_INTERNED)
				{
					...
					if(strlen(buffer) == length && IsTxtAsciiChar(buffer[0]))
					{
						...
						sprintf(temp, "<internStr index=\"%d\" length=\"%ld\" value=\"%s\"/>\0", m_StrIndexInList++, length, buffer);
					}
					else
					{
						sprintf(temp, "<internStr index=\"%d\" length=\"%ld\" value=\"binary\"/>\0", m_StrIndexInList++, length);
					}
				}
				else
				{			
					...
					{
						cout << "string in buffer : " << buffer << endl;
						sprintf(temp, "<str length=\"%ld\" value=\"%s\"/>\0", length, buffer);
					}
					else
					{
						sprintf(temp, "<str length=\"%ld\" value=\"binary\"/>\0", length);
					}
				}
				m_Writer.Write(temp);
				return strObject;
